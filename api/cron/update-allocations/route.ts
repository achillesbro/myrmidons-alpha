// api/cron/update-allocations/route.ts
...
export default async function handler(request: any, response: any) {
  ...
  if (expectedSecret && isManualTrigger) {
    ...
    const providedSecret = request.query?.secret;
  ...
  const results: { vaultId: string; status: string; error?: string }[] = [];
  ...
  for (const vaultId in VAULT_CURATOR_MAP) {
    ...
    try {
      ...
      results.push({ vaultId, status: 'success' });
  ...
  console.log('[Cron] Octav allocations update finished.');
