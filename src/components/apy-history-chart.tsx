...
import { useVaultBenchmarkAPI } from "../hooks/useVaultBenchmarkAPI";
...
export function ApyHistoryChart({ vaultAddress, chainId }: ApyHistoryChartProps) {
  ...
  const chartData = useMemo<ChartDataPoint[]>(() => {
    ...
    return netApyData.map((point) => {
      // Find corresponding benchmark data for this timestamp
      const benchmarkPoint = benchmarkData.find(bp => bp.x === point.x);
      const benchmarkValue = showBenchmark && benchmarkPoint ? benchmarkPoint.y : undefined;
  ...
  }, [netApyData, showBenchmark, benchmarkData]);
  ...
  if (loading || (showBenchmark && benchmarkLoading)) {
    return (
  ...
  if (error || (showBenchmark && benchmarkError)) {
    return (
      <div className="bg-[#FFFFF5] border border-[#E5E2D6] rounded-lg p-5">
        <div className="flex justify-between items-center mb-4">
          <h3 className="text-lg font-semibold text-[#00295B]">APY History</h3>
    ...
    <div className="bg-[#FFFFF5] border border-[#E5E2D6] rounded-lg p-5">
      ...
      <div className="flex justify-between items-center mb-4">
        ...
        <div className="flex items-center gap-3">
          <label className="flex items-center gap-2 text-xs text-[#101720]/70">
            <input
              type="checkbox"
              checked={showBenchmark}
              onChange={(e) => setShowBenchmark(e.target.checked)}
              className="w-3 h-3 rounded border-[#E5E2D6] text-[#8C7D57] focus:ring-1 focus:ring-[#8C7D57]"
            />
            Benchmark
          </label>
          <select
          ...
          >
            ...
            {showBenchmark && (
              <Line
                type="monotone"
                dataKey="benchmark"
                stroke="rgba(20, 23, 38, 0.4)"
                strokeWidth={1}
                strokeDasharray="2 2"
                dot={false}
                activeDot={{ r: 2, fill: "rgba(20, 23, 38, 0.4)" }}
              />
            )}
          </AreaChart>
        </ResponsiveContainer>
